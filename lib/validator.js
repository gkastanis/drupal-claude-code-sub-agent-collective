...
class CollectiveValidator {
  ...
  async validateInstallation() {
    ...
    const fileChecks = [
      ...
      { name: 'MCP configuration', path: '.mcp.json' },
    ...
    const mcpCheck = await this.validateMCPConfig();
    ...
    try {
      if (!await fs.pathExists(mcpConfigPath)) {
        return {
          ...
          passed: false,
          error: '.mcp.json does not exist'
        };
      }

      const mcpConfig = await fs.readJson(mcpConfigPath);
      const mcpManager = new MCPConfigManager();
      const validation = mcpManager.validateConfig(mcpConfig);

      if (!validation.valid) {
        return {
          name: 'MCP configuration validation',
          passed: false,
          error: `Invalid MCP configuration: ${validation.errors.join(', ')}`
        };
      }

      // Additional checks: verify Task Master is present (required)
      if (!mcpConfig.mcpServers || !mcpConfig.mcpServers['task-master']) {
        return {
          name: 'MCP configuration validation',
          passed: false,
          error: 'Task Master server is required but not found in .mcp.json'
        };
      }

      // Calculate and display resource usage info
      const resourceUsage = mcpManager.getResourceUsage(mcpConfig);
      const serverCount = Object.keys(mcpConfig.mcpServers).length;

      return {
        name: 'MCP configuration validation',
        passed: true,
        info: `${serverCount} MCP server(s) configured (${resourceUsage.memory} memory, ${resourceUsage.cpu} CPU)`,
        error: null
      };
      ...
      return {
        name: 'MCP configuration validation',
        passed: false,
        error: `MCP configuration error: ${error.message}`
      };
    }
  }
