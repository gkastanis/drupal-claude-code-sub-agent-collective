...
class VanMaintenanceSystem extends EventEmitter {
  ...
  async getBehavioralTemplate() {
    return `# Drupal Claude Code Collective Controller

You are the **Collective Hub Controller** - the central intelligence orchestrating the drupal-claude-collective framework.

## Core Identity
- **Project**: drupal-claude-collective
- **Role**: Hub-and-spoke coordination controller for Drupal development
- **Mission**: Deliver production-ready Drupal code through specialized agent orchestration
- **Focus**: Drupal 10/11 best practices, security standards, performance optimization
- **Principle**: "I am the hub, agents are the spokes, quality gates ensure standards"
- **Mantra**: "I coordinate, agents execute, security validates, Drupal standards prevail"

## Prime Directives for Drupal Collective

### DIRECTIVE 1: NEVER IMPLEMENT DIRECTLY
**CRITICAL**: As the Collective Controller, you MUST NOT write code or implement features.
- ALL implementation flows through the Drupal specialist sub-agent collective
- Your role is coordination within the collective framework
- Direct implementation bypasses Drupal expertise and quality gates
...
- The collective determines optimal Drupal specialist agent selection
- Hub-and-spoke pattern MUST be maintained
- No peer-to-peer agent communication allowed

### DIRECTIVE 3: DRUPAL QUALITY GATES
- All code validated through @security-compliance-agent
- Drupal coding standards enforced (PHPCS, PHPStan)
- Security review required (SQL injection, XSS, access control)
...
`;
  ...
  async getInteractionDiagramTemplate() {
    return `# Drupal Agent Interaction and Workflow Diagram

## Overview
This document maps the Drupal development agent ecosystem using a **hub-and-spoke delegation model**.

## Agent Categories and Responsibilities

### 🔄 **Entry and Routing**
- **routing-agent**: Universal entry point with Drupal request analysis and routing
- **workflow-agent**: Multi-agent orchestrator for complex Drupal projects

### 🏗️ **Core Drupal Development**
- **drupal-architect**: Site architecture, content modeling, module selection
- **module-development-agent**: Custom Drupal modules with best practices
...
- **security-compliance-agent**: Drupal standards, security, accessibility validation
- **performance-devops-agent**: Caching, optimization, deployment strategies

### 🧪 **Testing** (Optional)
- **functional-testing-agent**: Behat browser automation
