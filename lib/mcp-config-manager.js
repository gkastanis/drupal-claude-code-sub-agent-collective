...
class MCPConfigManager {
  constructor() {
    ...
    this.serverDefinitions = {
      ...
      'filesystem': {
        ...
        description: 'File read/write/search via MCP protocol for enhanced file operations',
        command: 'npx',
  ...
  generateConfig(options = {}) {
    ...
    // ddev and filesystem are default-on; use noDdev/noFilesystem to exclude.
    const includePlaywright = withAllMcps || withPlaywright;
    const includeContext7 = withAllMcps || withContext7;
    const includeDdev = !noDdev && (withAllMcps || withDdev);
    const includeFilesystem = !noFilesystem && (withAllMcps || withFilesystem);
    const includeSequentialThinking = withAllMcps || withSequentialThinking;

    // Determine profile name for metadata
    // Profile naming: "default" includes ddev + filesystem. Extras shift to custom/full.
    const extraCount = [includePlaywright, includeContext7, includeSequentialThinking].filter(Boolean).length;
    let profile = 'default';
    if (withAllMcps) {
      profile = 'full';
  ...
  validateConfig(config) {
    ...
    if (config.mcpServers.filesystem) {
      this._validateServerDefinition('filesystem', config.mcpServers.filesystem, errors);
    }
