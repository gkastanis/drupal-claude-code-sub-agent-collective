...
class CollectiveInstaller {
  ...
  /**
   * Display Drupal-themed banner
   */
  displayBanner() {
    console.log(chalk.blue(`
    💧 DRUPAL CLAUDE COLLECTIVE v${this.config.version}
    ─────────────────────────────────────────────
    AI-Powered Development for Drupal 10/11
`));
  }
  ...
  async performStandardInstallation() {
    ...
    this.displayMCPSummary();
    this.displayOptionalToolsGuidance();
  ...
  async performExpressInstallation() {
    ...
    console.log(chalk.gray('[DEBUG] Step 4.5: Configuring MCP servers'));
    await this.configureMCPServers();
  ...
  async createDirectories() {
    ...
    const dirs = [
      ...
      '.claude/commands',
      '.claude/skills',
      '.claude/skills/semantic-docs',
      '.claude/skills/semantic-docs/scripts',
      '.claude/skills/drupal-entity-api',
      '.claude/skills/drupal-service-di',
      '.claude/skills/drupal-caching',
      '.claude/skills/drupal-security-patterns',
      '.claude/skills/drupal-coding-standards',
      '.claude/skills/drupal-hook-patterns',
      '.claude/skills/twig-templating',
      '.claude/skills/agent-browser',
      '.claude/skills/discover',
      '.claude/skills/discover/scripts',
      '.claude/skills/prd',
      '.claude/rules',
      '.claude/agent-memory',
      '.claude/agent-memory/drupal-architect',
      '.claude/agent-memory/module-development-agent',
      '.claude/agent-memory/security-compliance-agent',
      '.claude/agent-memory/research-agent',
      '.claude/agent-memory/configuration-management-agent',
      '.claude-collective',
  ...
  async validateInstallation() {
    ...
    const checks = [
      ...
      { name: 'Skills directory', path: '.claude/skills' },
      { name: 'Rules directory', path: '.claude/rules' },
      { name: 'Agent memory directory', path: '.claude/agent-memory' },
  ...
  async configureMCPServers() {
    ...
    try {
      ...
      const mcpConfig = mcpManager.generateConfig({
        withPlaywright: this.options.withPlaywright || false,
        withContext7: this.options.withContext7 || false,
        withDdev: this.options.noDdev ? false : true,
        withFilesystem: this.options.noFilesystem ? false : true,
        noDdev: this.options.noDdev || false,
        noFilesystem: this.options.noFilesystem || false,
        withSequentialThinking: this.options.withSequentialThinking || false,
        withAllMcps: this.options.withAllMcps || false,
        version: this.config.version
      });
      ...
      const validation = mcpManager.validateConfig(parsedConfig);

      if (!validation.valid) {
        spinner.fail('MCP configuration validation failed');
      ...
      this.mcpResourceUsage = mcpManager.getResourceUsage(parsedConfig);
  ...
  displayMCPSummary() {
    if (!this.mcpConfig || !this.mcpResourceUsage) {
      return;
    }
    ...
    servers.forEach(serverName => {
      const serverDef = this.mcpConfig.mcpServers[serverName];
      const icon = serverName === 'task-master' ? '✅' : '📦';
      const required = serverName === 'task-master' ? ' (required)' : '';
      console.log(`${icon} ${serverName}${required}: ${serverDef.description}`);

      if (serverDef.warning) {
        console.log(chalk.yellow(`   ⚠️  ${serverDef.warning}`));
      }
    });

    console.log(chalk.gray(`\nTotal resource usage: ${this.mcpResourceUsage.memory} memory, ${this.mcpResourceUsage.cpu} CPU`));
    console.log(chalk.gray(`Profile: ${this.mcpConfig.meta.profile}`));

    if (!this.options.withAllMcps) {
      const tips = [];
      if (!this.options.withPlaywright) tips.push('--with-playwright (visual regression testing)');
      if (!this.options.withSequentialThinking) tips.push('--with-sequential-thinking (problem decomposition)');
      if (tips.length > 0) {
        console.log(chalk.green('\n💡 Tip: Optional MCP servers available:'));
        tips.forEach(tip => console.log(chalk.gray(`    ${tip}`)));
        console.log(chalk.gray('    --with-all-mcps (include everything)'));
      }
      console.log(chalk.gray('\n   Default servers (ddev, filesystem) can be excluded with --no-ddev, --no-filesystem'));
    }
  }
  ...
  displayNextSteps() {
    ...
    console.log('2. Generate semantic docs for your project (enables /discover and fast code navigation):');
    console.log(chalk.cyan('   In Claude Code: "Generate semantic documentation for this project"'));
    console.log(chalk.gray('   This runs semantic-architect-agent to create docs/semantic/ with'));
    console.log(chalk.gray('   business index, Logic IDs, and code mappings used by all agents.'));
    console.log('3. Validate installation: npx drupal-claude-collective validate');
    console.log('4. Try a request: "Create a custom block plugin that displays recent articles"');
  }

  displayOptionalToolsGuidance() {
    console.log(chalk.cyan('\n🔧 Optional tools to enhance installed skills:'));
    console.log(chalk.white('   agent-browser') + chalk.gray(' - CLI browser automation (used by testing agents)'));
    console.log(chalk.gray('     npm install -g agent-browser && agent-browser install'));
    console.log(chalk.white('   qmd') + chalk.gray(' - Fast document search (used by /discover skill, optional)'));
    console.log(chalk.gray('     npm install -g @nicholasgriffintn/qmd'));

    if (!this.options.noDdev) {
      console.log(chalk.white('   ddev-mcp') + chalk.gray(' - DDEV MCP server (enabled in your .mcp.json)'));
      console.log(chalk.gray('     See: https://github.com/ddev/ddev-mcp'));
    }

    console.log(chalk.gray('\n   These are not required. Skills fall back gracefully when tools are absent.'));
  }
}
