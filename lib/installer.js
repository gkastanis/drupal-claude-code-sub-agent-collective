...
`));
  ...
  async configureMCPServers() {
    ...
    try {
      ...
      const mcpConfig = mcpManager.generateConfig({
        ...
        withDdev: this.options.withDdev || false,
  ...
  displayMCPSummary() {
    ...
    if (!this.options.withAllMcps) {
      ...
      if (!this.options.withDdev) tips.push('--with-ddev (DDEV database/CLI integration)');
  ...
  displayOptionalToolsGuidance() {
    ...
    if (this.options.withDdev || this.options.withAllMcps) {
