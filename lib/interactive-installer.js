...
class InteractiveInstaller {
  ...
  displayBanner() {
    ...
    ╔═══════════════════════════════════════════════════════════╗
    ║                                                           ║
    ║      💧  DRUPAL CLAUDE COLLECTIVE  💧                     ║
    ║                                                           ║
    ║      AI-Powered Development for Drupal 10/11              ║
    ║                                                           ║
    ╚═══════════════════════════════════════════════════════════╝
`));
    console.log(chalk.gray(`    Version ${packageInfo.version} | 15 Specialized Agents | TDD Workflow\n`));
  }
  ...
  async smartMergeFlow(analysis) {
    ...
    // Backup options - single prompt with cancel option
  ...
  async forceOverwriteFlow(analysis) {
    ...
    const { forceAction } = await inquirer.prompt([{
      type: 'list',
      name: 'forceAction',
      message: 'Force overwrite - how do you want to proceed?',
      choices: [
        { name: 'Overwrite with backup (recommended)', value: 'backup' },
        { name: 'Overwrite without backup', value: 'no-backup' },
    ...
    return await this.executeForceOverwrite(analysis, forceAction === 'backup');
  }
